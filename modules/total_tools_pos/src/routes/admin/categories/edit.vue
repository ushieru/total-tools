<script setup>
import { GetCategoryById } from '@pos/gql/queries.gql'
import { UpdateCategoryById } from '@pos/gql/mutations.gql'
...
const { loading, onResult } = useQuery(GetCategoryById, { id: categoryId })
const { mutate: updateCategory } = useMutation(UpdateCategoryById)

const category = ref()

onResult(({ data }) => {
    if (!data) return
    if (!data.category) return router.push("/admin/categories")
    category.value = data.category
})

const dayIsChecked = (weekday) => {
    return category.value.availableDays.includes(weekday)
}
...
<template>
    ...
    <div v-if="category" class="card bg-base-200 shadow-xl">
        <div class="card-body">
            <h1 class="text-2xl">Editar Categoria <span class="font-bold">{{ category.id }}</span></h1>
            <form @submit.prevent="onSubmitUpdateCategory" class="grid grid-cols-1 lg:grid-cols-2 gap-2">
                ...
                <label class="form-control w-full">
                    ...
                    <input :value="category.availableFrom" type="date" name="available_from"
                ...
                <label class="form-control w-full">
                    ...
                    <input :value="category.availableUntil" type="date" name="available_until"
                        class="input input-bordered w-full" />
                </label>
                <label class="form-control w-full">
                    <div class="label">
                    ...
                    <input :value="category.availableFromHour" type="time" name="available_from_hour"
                        class="input input-bordered w-full" />
                </label>
                <label class="form-control w-full">
                    <div class="label">
                    ...
                    <input :value="category.availableUntilHour" type="time" name="available_until_hour"
                        class="input input-bordered w-full" />
                </label>
